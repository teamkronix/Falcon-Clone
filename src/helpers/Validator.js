  ...
  static validateConfiguration() {
    ...
    if (config.DASHBOARD.enabled) {
      if (!config.BOT_SECRET) {
        error("config: BOT_SECRET cannot be empty");
        process.exit(1);
      }
    ...
    if (config.MUSIC.ENABLED) {
      if (!config.SPOTIFY_CLIENT_ID || !config.SPOTIFY_CLIENT_SECRET) {
        warn("config: SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRET are missing. Spotify music links won't work");
      }
    ...
    if (!config.WEATHERSTACK_KEY) warn("config: WEATHERSTACK_KEY is missing. Weather command won't work");
    if (!config.STRANGE_API_KEY) warn("config: STRANGE_API_KEY is missing. Image commands won't work");
  }

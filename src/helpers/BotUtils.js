...
module.exports = class BotUtils {
  ...
  static async checkForUpdates() {
    ...
    if (!response.success) return error("snoww. (falcon) >>> versionCheck: Failed to check for bot updates");
    if (response.data) {
      if (
        require("@root/package.json").version.replace(/[^0-9]/g, "") >= response.data.tag_name.replace(/[^0-9]/g, "")
      ) {
        success("snoww. (falcon) >>> versionCheck: Your discord bot is up to date");
      } else {
        warn(`snoww. (falcon) >>> versionCheck: ${response.data.tag_name} update is available`);

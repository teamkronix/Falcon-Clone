...
module.exports = class BotClient extends Client {
  ...
  loadEvents(directory) {
    this.logger.log(`snoww. (falcon) >>> Loading events...`);
    let success = 0;
    let failed = 0;
    const clientEvents = [];
    ...
    this.logger.log(`snoww. (falcon) >>> Loaded ${success + failed} events. Success (${success}) Failed (${failed})`);
  }
  ...
  loadCommands(directory) {
    this.logger.log(`snoww. (falcon) >>> Loading commands...`);
    ...
    this.logger.success(`snoww. (falcon) >>> Loaded ${this.commands.length} commands`);
    this.logger.success(`snoww. (falcon) >>> Loaded ${this.slashCommands.size} slash commands`);
  ...
  loadContexts(directory) {
    this.logger.log(`snoww. (falcon) >>> Loading contexts...`);
    const files = recursiveReadDirSync(directory);
    for (const file of files) {
      try {
    ...
    this.logger.success(`snoww. (falcon) >>> Loaded ${userContexts} USER contexts`);
    this.logger.success(`snoww. (falcon) >>> Loaded ${messageContexts} MESSAGE contexts`);
  }
